...
function chooseUserAgent(ua = false) {
  ...
  const realUserAgentList = userAgentList[ua] || userAgentList.mobile.concat(userAgentList.pc)
...
const createRequest = (method, url, data = {}, options) => new Promise((resolve, reject) => {
  ...
  /** @type {import('axios').AxiosRequestConfig<string>} */
  let settings = {
    method,
    url,
    headers,
      ...
      if (purl.hostname) {
        const agent = tunnel[
          ...
          proxy: {
            host: purl.hostname,
            port: purl.port || 80,
            proxyAuth: purl.username && purl.password
  ...
  axios(settings)
    .then((res) => {
      ...
      try {
        ...
        answer.body.code = Number(answer.body.code || 0)
        answer.status = Number(answer.body.code || res.status)

        if (
          [201, 302, 400, 502, 800, 801, 802, 803].includes(answer.body.code)
        ) {
          // 特殊状态码
          answer.status = 200
        }
